<?php
    ...
    if(!empty($_GET['id']))
    {
        $products=$con->myQuery("SELECT * FROM products WHERE product_id=?",array($_GET['id']))->fetch(PDO::FETCH_ASSOC);
        if(empty($products)){
            ...
            <div class='col-md-12'>
              <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    ...
                    ?>
                    <li <?php echo $tab=="1"?'class="active"':''?>><a href="frm_products.php<?php echo !empty($products)?"?id={$products['product_id']}":''; ?>" >Product Details</a>
                    </li>
                    <li <?php echo empty($products)?'class="disabled"':''; ?> <?php echo $tab=="2"?'class="active"':''?> ><a href="?tab=2<?php echo !empty($products)?"&id={$products['product_id']}":''; ?>" <?php echo empty($products)?'onclick="alert(\''.$no_employee_msg.'\');return false;"':''; ?>>Suppliers</a>
                    </li>
                </ul>
